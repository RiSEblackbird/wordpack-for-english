...
class _OpenAILLM(_LLMBase):  # pragma: no cover - network not used in tests
    ...
    def complete(self, prompt: str) -> str:
        ...
        try:
            ...
            # Langfuse input にプロンプトのプレビューも含める
            with span(
            ...
            if ("unsupported parameter" in text1 or "not supported" in text1) and "max_output_tokens" in text1:
                try:
                    ...
                    with span(
                        trace=None if lf_trace is None else lf_trace(name="LLM call"),
                        name="openai.responses.create",
                        input={"model": self._model, "prompt_chars": len(prompt), "param":"max_tokens", "prompt_preview": prompt[:500]},
                    ) as _s:
            ...
            try:
                ...
                with span(
                    trace=None if lf_trace is None else lf_trace(name="LLM call"),
                    name="openai.responses.create",
                    input={"model": self._model, "prompt_chars": len(prompt), "json": False, "prompt_preview": prompt[:500]},
                ) as _s:
                ...
                with span(
                    trace=None if lf_trace is None else lf_trace(name="LLM call"),
                    name="openai.responses.create",
                    input={"model": self._model, "prompt_chars": len(prompt), "param":"max_completion_tokens", "prompt_preview": prompt[:500]},
                ) as _s:
