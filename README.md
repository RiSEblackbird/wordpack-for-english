...
- WordPack プレビューの読み込みに失敗した場合は、読み込み中表示を残さず、対象語・エラー内容・再試行・閉じる操作を同じモーダル内に表示します。
- Lexicon の保存済みWordPackは、上部検索、公開/非公開/生成済み/未生成チップ、カード/リスト表示切替で管理できます。カードには「開く」「音声」「語義」を常時表示し、削除や個別公開切替は右上メニューへまとめています
- 保存済みWordPackのヘッダーは、画面幅が狭い場合に縦並びとなり、検索・作成ショートカット・更新・表示切替が押しやすく表示されます
...
- デスクトップでは共通サイドバーを左側に表示し、折りたたむとアイコンだけのメニューとして本文領域を広げられます。Lexicon / Reader / Examples / Explore / Shelves / Quiz / Settings へ移動可能。900px以下では画面左上のハンバーガーボタンでサイドバーを開閉し、モバイルでは主要画面への下部ナビも表示されます
...
- 展開中のサイドバー下部に GitHub リンクと「ログアウト」ボタンを常設。どのタブからでもセッションを終了でき、共有端末の利用でも安全にサインアウトできます
...
## ゲスト閲覧モード
- **概要**: ログインせずに画面を閲覧できる読み取り専用モードです。ログイン画面の「ゲスト閲覧モード」ボタンから入り、展開中のサイドバー下部にある「ログアウト」ボタンで終了できます。
...
### 前提
- Python 3.13（CI と Codex ローカル環境セットアップは 3.13 系を前提にしています）
- Node.js 20.19.0+
...
- サインイン画面の「ゲスト閲覧モード」ボタンを押すとログイン不要でアプリの閲覧ができます。ゲスト中はサイドバー内にバッジが表示され、展開中のサイドバー下部にある「ログアウト」ボタンからゲスト閲覧を終了できます。ブラウザを再読み込みしても同じ状態が復元されます。
...
- 展開中のサイドバー下部にある「ログアウト」ボタン、または「設定」タブ下部のログアウトボタンから明示的にサインアウトできます。ログアウト時は `/api/auth/logout` へ通知し、バックエンドが HttpOnly の通常セッション Cookie とゲストセッション Cookie を失効させたうえで再びサインイン画面へ戻ります。万一バックエンドが応答しない場合はクライアント側で Cookie を削除するフォールバックが働きます。
...
- **例文生成・記事化が成功通知なのに記事/WordPackが増えない**: `/api/article/generate_and_import` は内部で例文2件を記事化しますが、両方のインポートが失敗すると 502 / `reason_code=CATEGORY_IMPORT_FAILED_ALL` を返すようになりました。Cloud Logging には `category_example_import_failed` と `category_generate_import_failed_all` が連続して出力されるため、失敗した例文インデックスと例外クラスを確認してから WordPack 側の例文を手動で記事化するか、設定（LLMモデルや reasoning/text）を見直して再実行してください。1件でも記事化できた場合は 200 を返しつつ `category_generate_import_partial_success` を警告ログに残すので、成功数と失敗数をログで把握できます。
