...
- Google Identity Services 側には `responseType: 'id_token token'` を要求し、implicit flow でも ID トークンを常に取得します。共有端末では `prompt: 'select_account'` によりアカウント選択ダイアログが毎回表示されるため、誤ったアカウントのまま処理が進むことを防ぎます。
