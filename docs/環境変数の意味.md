...
- 既定値: env.example は `wordpack-local` / `127.0.0.1:8080`（コメント付き）。コード側ではエミュレータ指定時に `local-emulator`、非本番では `wordpack-local` をフォールバックし、Cloud Firestore へ向かう場合のみ明示的なプロジェクト ID を要求する。
