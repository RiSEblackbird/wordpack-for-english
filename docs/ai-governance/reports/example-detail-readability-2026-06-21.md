...
- 証跡不足: 実ユーザーテストは未実施
- 残リスク: 省略記号や略語を含む英文では sentence split が完全ではない可能性がある。その場合は全文ペアまたは近い文単位表示になり、内容は失わない
...
| P2 | 文分割 | 略語などの sentence split は完全ではない | 一部の例文でペア粒度が粗くなる | 将来必要なら生成側で明示的な対応配列を返す | 未対応 |
...
- [x] その他: Browser metrics で文ペア数2、要点への品詞分解混入なし、horizontal overflow 0
...
| 全LLM出力パターンの網羅 | 生成文の表記揺れが広いため deterministic に網羅できない | 一部の解説で分類が完全でない可能性 | 生成側に構造化フィールドを追加する Issue を検討 |
