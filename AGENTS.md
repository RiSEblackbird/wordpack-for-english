...
リポジトリ変更を伴う作業では、実装・編集に入る前、または遅くとも PR 作成前に、対応する Issue を既存 Issue から特定する。既存 Issue がない場合は新規 Issue を作成する。Issue は作業の入口、PR はその実施証跡として扱う。

Issue 化の対象は、新機能、不具合修正、UI/UX 改善、設計変更、セキュリティ改善、認証・認可・権限変更、Cloud Run / Firebase / Firestore / 外部 API の運用調査、ドキュメント整備、ガバナンス変更、継続的に追跡すべき CI 失敗である。調査だけでも、後続作業・判断・運用証跡を残す必要がある場合は Issue を作るか既存 Issue に追記する。

Issue を省略してよいのは、既存 PR の review comment への局所修正、同一 PR 内の CI 失敗修正、typo / リンク切れ / コメント修正、既存 Issue に完全に包含される追加作業、リポジトリ変更を伴わない軽微な質問回答に限る。省略した場合は、PR 本文または最終報告に `Issue: N/A` と省略理由を書く。

PR 本文の Issue 欄には、完全解決なら `Closes #123`、不具合修正なら必要に応じて `Fixes #123`、完全解決の別表現なら `Resolves #123`、部分対応なら `Refs #123`、段階対応なら `Part of #123`、関連するが解決しない場合は `Related to #123` を書く。GitHub の自動クローズ対象は `Closes` / `Fixes` / `Resolves` 系だけであり、`Refs` / `Part of` / `Related to` は自動クローズではない。

複数 Issue がある場合でも主 Issue を 1 つ決め、無関係な Issue を大量に紐付けない。複数 Issue を完全に閉じる場合は、それぞれに `Closes #123, Closes #456` のように完全な closing keyword を書く。default branch 以外を base にする PR では、GitHub の closing keyword による自動クローズに頼らない。

調査で判明した事実、受け入れ条件の変更、非対象、残るリスク、後続作業は、PR 作成前に Issue 本文または Issue コメントへ反映する。ただし、秘密情報、個人情報、本番ログ原文、不要な運用識別子は Issue に残さない。
...
- PR 本文には、Issue、変更内容、保持した既存挙動、検証結果、未実行項目、Codex review / review thread 確認結果、残るリスクを記載する。
