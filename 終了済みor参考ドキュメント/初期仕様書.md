...
- 変更採否は次の5基準の3/5以上で決定：①20秒カード ②自作文促進 ③SRS継続 ④不確実性の可視化 ⑤発音理解の支援。
...
- `GET /api/review/today`（5枚）
- `POST /api/review/grade`（採点→SRS更新）
- `POST /api/word`（任意登録）
- `GET /healthz`
...
## 5. SRS（簡易SM-2）
- `ease ∈ [1.5,3.0]`、`correct/partial/wrong`で間隔更新、次回は`now + interval`。
...
### 2.1 カードパネル（「周辺知識」＋「今日の復習」）
**レイアウト（縦一列）**
- 入力行：`lemma`（テキスト）＋`pos`（任意）＋「取得」ボタン  
- 結果：**周辺知識カード**（1枚表示）
- 復習：5件リスト（クローズ/パラフレーズ）＋採点ボタン
...
- 復習一覧：`GET /api/review/today` → 各アイテムを表示  
- 採点：`POST /api/review/grade`
