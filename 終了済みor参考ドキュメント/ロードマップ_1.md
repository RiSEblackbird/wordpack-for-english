...
- [x] M4 WordPack保存とカードUIの仕上げ（3–4日）
  - [x] 保存済みWordPackの一覧・詳細・削除APIを整備
  - [x] フロントのカードUIを保存APIと連携させ、生成→保存→再生成の循環を確認
...
- 構成
  ...
  - ルータ: `src/backend/routers/{word,article,config,health}.py`
  - モデル: `src/backend/models/{word,text,sentence}.py`
...
- 配線統一（最初に実施）
  ...
  - `WordPackPanel.tsx`: `/cards/*` から `/api/word/*` に統一（生成/保存/再生成/削除）
...
## 受け入れ基準（Doneの定義）
- 自作文・注釈・WordPack の主要機能が実行でき、各レスポンスに `citations`（RAG導入後）または欠落明示がある
- p95: 注釈≤5s、WordPack≤5s（M3以降）
...
まとめ
- フロントはまずAPIパス整合と共通フェッチ基盤を整備。バックエンドは自作文/注釈→WordPack→RAGの順で拡張。各段階で欠落明示と`confidence`運用を徹底し、CIテストを並走させて確実に前進します。