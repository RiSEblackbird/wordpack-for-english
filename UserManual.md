...
### A-2.5. ログインとユーザーロール
- ログインは Google アカウントで行います。システム手前のリバースプロキシ/IAP がログイン後のメールアドレスを `X-User-Email` などのヘッダに挿入し、バックエンドがロールを判定します。
- 管理者ロール（admin）はすべての生成機能を利用できます。閲覧専用ロール（viewer）は以下のボタンがグレーアウトし、クリックすると「閲覧ユーザーはAI生成機能を利用できません」という警告が表示されます。
  - WordPack パネルの「生成」「再生成」「WordPackのみ作成」ボタン
  - WordPack 一覧内の「生成」ショートカット
  - 文章インポートパネルの「インポート」「生成してインポート」ボタン
  - 文章一覧の「自動生成して記事化」操作（カテゴリ生成を含む）
  - 例文カードの「音声」および WordPack/文章詳細の音声ボタン
- 管理者/閲覧専用の割当は `.env` の `ADMIN_EMAIL_ALLOWLIST` / `VIEWER_EMAIL_ALLOWLIST`（またはドメイン版）で設定します。対象外のアカウントは既定値 `USER_ROLE` のロールになります。
- ログイン済みでも各操作が無効な場合は、ご自身の Google アカウントが閲覧専用に設定されていないか管理者へご確認ください。
...
- 「音声」ボタン: OpenAI gpt-4o-mini-tts で原文を音声再生（音量は端末で調整）。詳細モーダルでは原文・日本語訳それぞれの見出し右側にもボタンがあり、どちらの文も個別に再生できます。
  - 閲覧専用ロールではこれらの音声ボタンがグレーアウトし、クリックすると利用不可のメッセージが表示されます。
...
- ※閲覧専用ロールの場合、上記ステップのうち生成関連ボタンはグレーアウトしており実行できません（サイドバー内に警告が表示されます）。
...
- 閲覧専用ロールではこれらの生成・インポートボタンがグレーアウトし、「閲覧ユーザーはAI生成機能を利用できません」と表示されます。
...
1. ブラウザで WordPack ポータルを開くと最初にログイン画面が表示されます。
2. `Googleでログイン` ボタンを押すと Google の認証画面が開きます。
3. WordPack に使用するアカウントを選択すると、認証が完了してダッシュボードが表示されます。
4. ログイン後はブラウザにセッションが保持され、次回のアクセスも自動的に認証済みになります。

### ログアウトする

1. 画面右上のプロフィールメニューから `ログアウト` を選択します。
2. ログアウトするとログイン画面へ戻り、再度 Google ログインが必要になります。
