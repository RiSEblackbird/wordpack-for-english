...
- Alt+1..7: 主要画面へ移動（1=Lexicon, 2=Settings, 3=Reader, 5=Examples, 6=Explore, 7=Shelves）
  - `/`: 主要入力欄へフォーカス
- 通知表示: Lexicon / Reader / Examples / Explore / Shelves / Settings / WordPack詳細の各画面では、右側または本文下へ回り込む「生成キュー」に進行中/完了した生成が残ります。進行中はスピナーと進捗バー、完了時はチェック付きの履歴として表示され、進行中/完了チケットが更新されると約2秒間、枠が青白く明滅します。WordPackを特定できる完了履歴はカード自体をクリックしてプレビューを開けます。必要に応じて履歴を消去できます。ページ移動/リロード後も表示は維持されます。生成/再生成/削除が完了すると保存済み一覧が自動で更新されます。
...
- 例文表示:
  ...
  - 長い解説は「要点」を先に表示し、品詞分解は「品詞分解を表示」から必要な時だけ展開できます。
