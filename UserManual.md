...
### A-4. 画面の見かた（共通）
- ヘッダー: 画面左上に固定されたハンバーガーボタン（メニューの開閉）とアプリ名 `WordPack`
- サイドバー: 「WordPack / 文章インポート / 例文一覧 / 設定」。ハンバーガーボタンを押すと左側からスッとスライド表示され、任意の項目を押しても開いたままです。閉じたいときはハンバーガーボタン（ヘッダー左上）を再度押します。サイドバーが開いている間は左端に固定され、十分な横幅がある場合はメイン画面の位置を保ったまま余白内で並びます。スペースが不足する場合のみメイン画面が自動でスライドし、UIが重ならない横並び表示になります。
