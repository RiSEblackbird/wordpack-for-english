...
- 表示内容: 英語タイトル / 英語本文 / 日本語訳 / 解説（日本語訳の直下に表示） / 作成日時・更新日時 / 生成所要時間 / 生成カテゴリ / AIモデル・パラメータ / 関連WordPack
- 関連WordPack一覧の直前には、作成日時・更新日時・生成所要時間・生成カテゴリ・AIモデル/パラメータを2列レイアウトでまとめたメタ情報が表示されます（未記録の項目は「未記録」「未指定」などの表示になります）。
