...
- サイドバー: 「Lexicon / Reader / Examples / Explore / Shelves / Settings」。十分な横幅では左側に固定表示され、ユーザー情報、GitHubリンク、ログアウトボタンも下部にまとまります。900px以下ではハンバーガーボタンを押すと左側から即座に表示され、閉じたいときはハンバーガーボタンを再度押すか、サイドバー外側の半透明な背景をタップ、または Esc キーで閉じられます。
...
- Alt+1..7: 主要画面へ移動（1=Lexicon, 2=Settings, 3=Reader, 5=Examples, 6=Explore, 7=Shelves）
  - `/`: 主要入力欄へフォーカス
- 通知表示: Lexicon では右側の「生成キュー」に進行中/完了した生成が残ります。進行中はスピナーと進捗バー、完了時はチェック付きの履歴として表示され、必要に応じて履歴を消去できます。その他の画面では右下に小さな通知カードを表示します。ページ移動/リロード後も表示は維持されます。生成/再生成/削除が完了すると保存済み一覧が自動で更新されます。
...
- 「語義」ボタンで語義タイトルを表示/非表示に切り替え（カード表示では見出し語の下に表示されます）。カードの操作列には「開く」「音声」「語義」が並び、未生成カードでは「生成」も表示されます。削除、個別の公開/非公開切替、未生成カードの生成は右上のその他メニューからも実行できます。
  ...
  - 不要ならその他メニューまたは選択バーの「削除」で削除します。削除を押すと画面中央に「【選択したWordPackの見出し語（例: converge）】について削除しますか？」という確認ダイアログが開き、「はい」「いいえ」で実行可否を選びます。
  - 各カードには `学 {学習済み回数}` `難 {確認のみ回数}` バッジが表示されます。リスト表示では従来どおり `学` / `確` のバッジで確認できます。WordPackプレビューの学習記録ボタンで更新すると即座に値が反映されます。
