...
- サイドバー: 「WordPack / 文章インポート / 例文一覧 / 設定」。ハンバーガーボタンを押すと左側からスッとスライド表示され、任意の項目を押しても開いたままです。閉じたいときはハンバーガーボタン（ヘッダー左上またはサイドバー上部）を再度押します。サイドバーが開いている間は、その幅に合わせて本画面の表示領域が自動的に縮みます。
