...
- Alt+1..7: 主要画面へ移動（1=Lexicon, 2=Settings, 3=Reader, 5=Examples, 6=Explore, 7=Shelves）
  - `/`: 主要入力欄へフォーカス
- 通知表示: Lexicon / Reader / Examples / Explore / Shelves / Settings / WordPack詳細の各画面では、右側または本文下へ回り込む「生成キュー」に進行中/完了した生成が残ります。進行中はスピナーと進捗バー、完了時はチェック付きの履歴として表示され、必要に応じて履歴を消去できます。ページ移動/リロード後も表示は維持されます。生成/再生成/削除が完了すると保存済み一覧が自動で更新されます。
...
- 「例文生成・記事化完了」と通知されたのに記事や WordPack が増えていない: 生成された2件の例文すべてが記事化に失敗した場合は生成キューの履歴がエラーに変わり、モーダルにも「例文を記事化できませんでした（reason_code=CATEGORY_IMPORT_FAILED_ALL）」という内容が表示されます。カテゴリ用に作成された WordPack の例文自体は保存済みなので、WordPackタブから該当語彙を開いて例文カードの「記事化」ボタンで個別に記事化するか、モデルや reasoning/text の設定を調整してから再度「例文を生成して記事化」を実行してください（片方だけ失敗した場合は 1 件だけ記事が追加され、通知には「一部失敗」と記録されます）。
