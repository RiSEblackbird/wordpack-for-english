...
- サイドバー: 「WordPack / 文章インポート / 例文一覧 / 設定」。ハンバーガーボタンを押すと左側からスッとスライド表示され、任意の項目を押しても開いたままです。閉じたいときはハンバーガーボタン（ヘッダー左上）を再度押します。サイドバーが開いている間は、サイドバー幅が既存の左右余白より大きいときにその差分だけメイン画面の左端を右へ寄せ、UIが重ならないように自動調整されます。
