...
- サービス起動（別ターミナル）:
  ...
  ```
ヒント（APIキー/出力）:
- 実行時は `.env` に `OPENAI_API_KEY` を設定。
- テスト/CI はモックのため不要。
- 本番は `STRICT_MODE=true` 推奨、開発は `false` で許容。
- 情報欠落時は `LLM_MAX_TOKENS` を増加（推奨1500、1200–1800）。
...
新ボタン（生成の右）:
- 「WordPackのみ作成」… 内容生成せず空のWordPackを保存。後から「再生成」で中身を埋められます。
...
セルフチェック:
- 「学習カード要点」に3秒のぼかし（自動/クリックで解除）。

レンマ直採点（PR2/PR3）:
- ×/△/○ で採点。未登録なら `id=w:<lemma>` を自動作成し裏面に「学習カード要点」を設定。
- ショートカット: `1/J=×`, `2/K=△`, `3/L=○`。設定で「採点後に自動で次へ」。

登録状況とSRSメタ:
- 登録済みなら `repetitions/interval_days/due_at` を表示。未登録は「未登録」。採点で自動登録・更新。
...
進捗の見える化（PR4）:
- 「今日のレビュー済/残り」、最近5語リンク、完了時に簡易サマリを表示。

単語アクセス導線（PR5）:
- 対比/共起/インデックス（最近・よく見る）からクリックで入力欄に反映。
...
- 付加メタ（任意）: `category`, `llm_model`, `llm_params`。UIは `en/ja/grammar_ja` のみ参照。

引用と確度（PR5）:
- 引用: 生成情報の参照元（LLMは `{"source":"openai_llm"}`）。
- 確度: `low/medium/high`。生成品質・整合性に基づき調整。
...
- **削除**: 「削除」で当該WordPackを削除。

例文の個別削除（新）:
- モーダル内の各例文に「削除」。`DELETE /api/word/packs/{id}/examples/{category}/{index}`。削除後は自動再読込。

内部実装メモ（性能改善・参考）:
- 例文はDBで別テーブルに正規化され、部分読み込み・部分削除が高速になりました（APIの入出力は従来どおり `examples` を含む完全な `WordPack`）。
...
- 例外監視（任意）: Sentry DSN 設定があれば重大エラーを送信。
