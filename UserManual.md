...
例文の個別削除（新機能）:
- 「保存済み」タブでカードをクリックしてモーダルを開き、`例文` セクションの各カードにある「削除」ボタンで、当該例文のみを削除できます（保存済みのWordPackに限る）。
- サーバAPIは `DELETE /api/word/packs/{id}/examples/{category}/{index}`（`category`: `Dev|CS|LLM|Business|Common`、`index`: 0始まり）。
- 削除後は自動的に最新状態へ再読込されます。
