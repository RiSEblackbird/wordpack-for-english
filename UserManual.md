...
- 「保存済み」タブで一覧表示・管理できます（表示/再生成/削除）

### A-4. 画面の見かた（共通）
- ヘッダー: アプリ名 `WordPack`
- ナビゲーション: 「カード / 文 / アシスト / WordPack / 保存済み / 設定」
- 初期表示: 「保存済み」タブ（保存済みの一覧）
- キーボード操作:
  - Alt+1..6: タブ切替（1=カード, 2=文, 3=アシスト, 4=WordPack, 5=保存済み, 6=設定）
  - `/`: 主要入力欄へフォーカス
- ローディング表示: 操作に応じたメッセージと経過時間を表示
  - 例）生成中 / 再生成中 / 保存済み読み込み中 / 採点記録中
- エラー表示: 画面上部に赤帯で表示
...
### A-10. 保存済み WordPack パネル
1) 「保存済み」を選択
2) 生成済みの WordPack をカード形式で一覧表示
3) カードをクリックで大きめのモーダルでプレビュー
4) 不要なら「削除」で削除（必要に応じて再生成も可能）
5) 多い場合は画面下部のページネーションで移動（現在件数/総件数を表示）
...
### A-12. 制約・既知の事項（ユーザー向け）
- UI文言は今後統一予定
- 復習（SRS）は自動で保存されます（初回起動時に数枚のカードが用意されます）

### A-13. トラブルシュート（一般ユーザー向け）
