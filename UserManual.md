...
- 保存済みWordPack一覧（同ページ下部）:
  ...
  - 不要なら「削除」で削除（必要に応じて再生成も可能）。削除を押すと画面中央に「【WordPack】について削除しますか？」という確認ダイアログが開き、「はい」「いいえ」で実行可否を選びます。
...
- WordPackカードの「削除」ボタンを押すと画面中央に「【WordPack】について削除しますか？」という確認ダイアログが開き、「はい」「いいえ」で削除可否を決定します。
...
6. 「インポート済み文章」一覧から、保存済みの文章を再表示できます。不要になった記事は削除してください（削除ボタンを押すと画面中央に「【文章】について削除しますか？」が表示され、「はい」「いいえ」で判断します）。
