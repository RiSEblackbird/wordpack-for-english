...
- 表示内容: 英語タイトル / 英語本文 / 日本語訳 / 解説（日本語訳の直下に表示） / 関連WordPack
...
3. モデルとパラメータを選択します。
   - モデル: 「gpt-5-mini / gpt-4.1-mini / gpt-4o-mini」から選択
   - gpt-5-mini を選ぶと `reasoning.effort` と `text.verbosity` の選択欄が表示されます（推論系）。
...
補足: 「生成＆インポート」ボタン（カテゴリ指定で例文を自動生成して記事化）でも、上記で選択したモデル/パラメータが反映されます。gpt-5-mini は `reasoning`/`text`、その他は `temperature` が送信されます。
