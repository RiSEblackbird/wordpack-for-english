...
- ゲスト中はサイドバー内に **「ゲスト閲覧モード」バッジ** が表示され、いまは閲覧専用であることを示します。展開中のサイドバー下部にある「ログアウト」ボタンからゲスト閲覧を終了でき、ブラウザを再読み込みしてもログアウト前は同じ状態が復元されます。
...
- ログアウトしたい場合は展開中のサイドバー下部にある「ログアウト」ボタンを押すか、「設定」タブ最下部のログアウトボタンを押してください。押下するとバックエンドへ `/api/auth/logout` リクエストが送信され、サーバー側で HttpOnly の通常セッション Cookie とゲストセッション Cookie が失効します。まれにサーバー応答が得られない場合はクライアント側で Cookie を削除するフォールバックが働き、必ずサインイン画面へ戻ります。
...
### A-4. 画面の見かた（共通）
- ヘッダー: デスクトップでは左サイドバーを表示し、画面上部には Lexicon の見出し、検索、作成ショートカットを優先して表示します。900px以下の狭い画面では画面左上端にハンバーガーボタンが出て、サイドバーを開閉できます。
- サイドバー: 「Lexicon / Reader / Examples / Explore / Shelves / Quiz / Settings」。十分な横幅では左側に固定表示され、上部の「折りたたむ」ボタンでアイコンだけのメニューにできます。折りたたみ中も主要メニューは使え、再度「展開」するとユーザー情報、GitHubリンク、ログアウトボタン、音声コントロール、ページ固有操作が表示されます。900px以下ではハンバーガーボタンを押すと左側から即座に表示され、閉じたいときはハンバーガーボタンを再度押すか、サイドバー外側の半透明な背景をタップ、または Esc キーで閉じられます。
...
- 画面構成:
  ...
  - 右列: 選択中 Quiz の本文、設問、採点サマリ、解説を表示します。狭い画面では縦に並びます。「本文/問題を広げる」で生成フォームと保存済み一覧を一時的に隠し、本文・問題に集中できます。
- 操作:
  - 「生成開始」で Quiz 生成ジョブを開始します。完了すると一覧が更新され、生成された Quiz が開きます。
  - 選択中 Quiz の「本文/問題を広げる」を押すと、本文と問題が画面幅を広く使います。戻る場合は「3カラムに戻す」を押します。
