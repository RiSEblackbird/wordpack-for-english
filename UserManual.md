...
- サイドバー: 「WordPack / 文章インポート / 例文一覧 / 設定」。ハンバーガーボタンを押すと左側からスッとスライド表示され、任意の項目を押しても開いたままです。閉じたいときはハンバーガーボタン（ヘッダー左上）を再度押します。サイドバーが開いている間は左端に固定され、メイン画面は残り幅へ自動で収まるためUIが重ならない横並び表示になります。
